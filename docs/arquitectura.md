...
| `moodle3` · leer intervenciones de un foro | `connectors/moodle3` | **Implementado con dos dialectos.** En Moodle 3.7+ usa `mod_forum_get_forum_discussions` y `mod_forum_get_discussion_posts` (las anteriores quedaron obsoletas en 3.7 y los sitios modernos ya no dejan añadirlas al servicio web, que fue el fallo del piloto); si el catálogo del token sólo declara las antiguas, usa `…_paginated` y `…_forum_discussion_posts`. Produce **como mucho una entrega por debate**: la del mensaje raíz, y sólo si nadie distinto del autor ha respondido —Vega contesta la primera duda sin responder, no todas—. |
...
| Almacén de binarios | `apps/api/src/storage/` | **Implementado para las entregas** (ADR 0012): el fichero descargado se guarda en `STORAGE_ROOT` y su ruta relativa en `submissions.storage_path`; `page_count` se cuenta del PDF de verdad con `pdf-lib`. La ruta autenticada de original lo sirve y pdf.js lo renderiza en la revisión. **Los ficheros de contexto de una actividad siguen sin almacén** (`activity_files.storage_path` sigue a `null`). |
...
| `POST /api/batch/run` | `routes/batch.ts` | Devuelve `202` inmediatamente y ejecuta el trabajo fuera de la petición. La orquestación durable por fases de Anthropic Batches sigue siendo el siguiente paso del transporte. |
| `GET /api/health` | `routes/health.ts` | Verifica la base de datos (`SELECT 1`, y 503 si falla) y reporta proveedor, transporte, modelos efectivos y conector leídos de `app_settings`; no prueba credenciales externas. |
